...
int main(int argc, char** argv) {
	...
	rsint.set_domain(d) = space.get_domain(d)->mult_r(space.get_domain(d)->mult_sin_theta(one(d))) ;
      ...
      double posmax = 1 ;
      double fmax = 0.05 ; 
      double sigmax = 2*posmax*posmax;
      double sigmaz = sigmax ;
      double vmax = fmax ;
      
      Scalar phi(space) ;
      for (int d=0 ; d<ndom-1 ; d++)
	phi.set_domain(d) = vmax * exp(-pow(space.get_domain(d)->get_cart(1), 2)/sigmax) *  exp(-pow(space.get_domain(d)->get_cart(2),2)/ sigmaz); 
